...
Poly Merge(int l, int r) {
  ...
  int m = (l + r) / 2;
  auto left = Merge(l, m);
  auto right = Merge(m + 1, r);
  return Multiply(left, right);
}

vector<Type> fact(N + 1);

Type GetSum(const Poly& p, int n) {
  Long result = 0;
  for (int i = 0; i < n; i++) result = Add(result, Mul(p[i], Mul(fact[i], fact[n - i])));
  return result;
}
...
int main(void) {
  ...
  while (tt--) {
    ...
    Long result = Mul(GetSum(Merge(0, n - 1), n), Pow(fact[n], MOD - 2));
    cout << result << '\n';
  }
  return 0;
}
