struct Punto {
	...
	bool operator != (Punto p) { return not (*this == p); }
	Punto operator + (Punto p) { return Punto(x + p.x, y + p.y); }
	Punto operator - (Punto p) { return Punto(x - p.x, y - p.y); }
	Punto operator * (long double k) { return Punto(x * k, y * k); }
	Punto operator / (long double k) { return Punto(x / k, y / k); }
	long double operator * (Punto p) { return x * p.x + y * p.y; } // Producto interno euclideano
//	Punto operator ^ (Punto p) { return Punto(y * p.z - z * p.y, z * p.x - x * p.z, x * p.y - y * p.x); }
	...
	// Lo siguiente solo funciona en geometria plana
